...
#+END_SRC
* Org-pomodoro 🍅
Mode to apply the [[https://francescocirillo.com/pages/pomodoro-technique][Pomodoro technique]] when using org-agenda.
...
In `Org-Agenda`, `P` on any entry will clock-in and start a `org-pomodoro` session.
