...
(defcustom eigengrau-diff-mode 'normal
  "Sets the level of highlighting to use in diff-like modes."
  :type 'symbol
  :options '(high normal low)
  :group 'lt)

(defcustom eigengrau-bold t
  "Stops Eigengrau from displaying bold when nil."
  :type 'boolean
  :group 'lt)

(defcustom eigengrau-underline t
  "Stops Eigengrau from displaying underlines when nil."
  :type 'boolean
  :group 'lt)

(defcustom eigengrau-italic t
  "Stops Eigengrau from displaying italics when nil."
  :type 'boolean
  :group 'lt)

(defcustom eigengrau-contrast 'normal
  "Stick with normal! It's been carefully tested. Setting this option to high or
low does use the same Eigengrau palette but simply shifts some values up or
...
the \"Gen RGB\" column in eigengrau-definitions.el to improve them further."
  :type 'boolean
  :group 'lt)
...
(defvar eigengrau-colors           ; ANSI(Eigengrau terminal)
  ;; name     sRGB      Gen RGB   256       16              8
  '((base03  "#161616" "#161616" "#202020" "brightblack"   "black")
    ...
    (base0   "#b4b4b4" "#b4b4b4" "#b4b4b4" "brightblue"    "blue")
    ...
    (red     "#cc3333" "#cc3333" "#ff3333" "red"           "red")
    (magenta "#FF1F69" "#FF1F69" "#FF1F69" "magenta"       "magenta")
    (violet  "#BB99EE" "#BB99EE" "#ccaaff" "brightmagenta" "magenta")
...
(defun eigengrau-color-definitions (mode)
  (noct-flet ((find-color (name)
                          (let* ((index (if window-system
                                            (if eigengrau-degrade
                                                3
                                              (if eigengrau-broken-srgb 2 1))
                                          (case (display-color-cells)
                                            (16 4)
                                            (8  5)
                                            (otherwise 3)))))
                            (nth index (assoc name eigengrau-colors)))))
             (let ((base03      (find-color 'base03))
                   ...
                   (bold        (if eigengrau-bold 'bold 'normal))
                   (bright-bold (if eigengrau-bold 'normal 'bold))
                   (underline   (if eigengrau-underline t nil))
                   (opt-under   nil)
                   (italic      (if eigengrau-italic 'italic 'normal)))
               (let ((back base03))
                 (cond ((< (display-color-cells) 16)
                        (setf back nil))
                       ((eq 'high eigengrau-contrast)
                 ...
                 (let ((bg-back   `(:background ,back))
                   ...
                   `((;; basic
                      ...
                      (mode-line  ; StatusLine
                       ((t (,@fg-base0 ,@bg-base01 :box nil)))) ; ,@fmt-revbb
                      (mode-line-inactive ; StatusLineNC
                       ((t (,@fg-base02 ,@bg-base01  :box nil))))

                      (region ((t (:background "#373b41")))) ; Visual
                      ...
                      (font-lock-doc-face ((t (,@fmt-ital ,@fg-cyan)))) ; Comment
                      ...
                      (font-lock-preprocessor-face ; PreProc
                       ((t (,@fmt-none ,@fg-violet)))) ;; for clj Java interop
                      ...
                      (show-paren-match ((t (,@fmt-bold ,@fg-cyan ,@bg-base02 ,@fmt-revr))))
                      ...
                      (erc-nick-defaueigengrau-face ((t (,@fmt-none ,@fg-cyan))))
...
(defmacro create-eigengrau-theme ()
  (let* ((theme-name 'eigengrau)
         (defs (eigengrau-color-definitions 'dark))
         (theme-vars (mapcar (lambda (def) (list (car def) (cdr def)))
                             (second defs)))
         (theme-faces (first defs)))
    `(progn
       (deftheme ,theme-name ,eigengrau-description)
